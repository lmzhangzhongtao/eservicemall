...
import com.caspar.eservicemall.common.constant.seckill.SeckillConstant;
...
import com.caspar.eservicemall.common.to.seckill.SeckillSessionWithSkusTO;
import com.caspar.eservicemall.common.to.seckill.SeckillSkuRedisTO;
import com.caspar.eservicemall.common.to.seckill.SkuInfoTO;
import com.caspar.eservicemall.common.utils.R;
import com.caspar.eservicemall.common.vo.MemberResponseVo;
import com.caspar.eservicemall.common.vo.seckill.SeckillSkuVO;
import com.caspar.eservicemall.seckill.feign.CouponFeignService;
import com.caspar.eservicemall.seckill.feign.ProductFeignService;
...
import static com.caspar.eservicemall.common.constant.seckill.SeckillConstant.SECKILL_CHARE_KEY;
    ...
    private void saveSessionInfos(List<SeckillSessionWithSkusTO> sessions) {
        if (!CollectionUtils.isEmpty(sessions)) {
            sessions.stream().forEach(session -> {
                ...
                System.out.println("saveSessionInfos 场次的key:"+key);
                // 2.判断场次是否已上架（幂等性）
    ...
    private void saveSessionSkuInfo(List<SeckillSessionWithSkusTO> sessions) {
        if (!CollectionUtils.isEmpty(sessions)) {
            ...
            if (info.getCode() == 0) {
                ...
                // 绑定秒杀商品hash
                BoundHashOperations<String, Object, Object> operations = redisTemplate.boundHashOps(SECKILL_CHARE_KEY);
                // 1.遍历场次
                sessions.stream().forEach(session -> {
                    // 2.遍历商品
                    session.getRelationSkus().stream().forEach(seckillSku -> {
                        ...
                        if (!operations.hasKey(skuKey)) {
                            // 未上架
                            System.out.println("当前场次商品未上架，准备上架:"+skuKey);
                            // 3.封装商品信息
    ...
    public String kill(String killId, String key, Integer num) throws InterruptedException {
        ...
        if (currentTime >= startTime && currentTime <= endTime) {
            ...
            if (randomCode.equals(key)) {
                // 获取每人限购数量
               // BigDecimal seckillLimit = skuInfo.getSeckillLimit();
                Integer seckillLimit=skuInfo.getSeckillLimit().intValue();
                // 获取信号量
